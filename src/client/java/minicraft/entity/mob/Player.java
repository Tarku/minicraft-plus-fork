...
public class Player extends Mob implements ItemHolder, ClientTickable {
	...
	public void tick() {
		...
		if (input.inputDown("STAIRSDOWNCHEAT"))
		{
			level.setTile(x / 16 + dir.getX(), y / 16 + dir.getY(), Tiles.get("Stairs Down"));
			World.levels[World.currentLevel - 1].setTile(x / 16 + dir.getX(), y / 16 + dir.getY(), Tiles.get("Stairs Up"));
		}
		if (input.inputDown("STAIRSUPCHEAT"))
		{
			level.setTile(x / 16 + dir.getX(), y / 16 + dir.getY(), Tiles.get("Stairs Up"));
			World.levels[World.currentLevel + 1].setTile(x / 16 + dir.getX(), y / 16 + dir.getY(), Tiles.get("Stairs Down"));
		}

		if (input.inputDown("BEDTPCHEAT"))
		{
			//World.levels[World.currentLevel].remove(this);

			World.currentLevel = 3;
			this.level = World.levels[World.currentLevel];

			World.levels[World.currentLevel].add(this);
		...
		if (Game.getDisplay() == null && !Bed.inBed(this)) {
			...
			if ((activeItem == null || !activeItem.used_pending) && (input.inputPressed("attack")) && stamina != 0 && onFallDelay <= 0) { // This only allows attacks when such action is possible.
				if (!potioneffects.containsKey(PotionType.Energy)) payStamina(1);
				staminaRecharge = 0;
	...
	public int getLightRadius() {
		int r = 32; // The radius of the light.
